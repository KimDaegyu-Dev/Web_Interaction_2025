...
//                     next_syllables.set(idx, match total_n { 2 | 3 => true, _ => false });
//                 } else { // Rule for components
//                     let consonant_survives = self.consonants[idx] && (total_n == 2 || total_n == 3);
//                     let vowel_survives = self.vowels[idx] && (total_n == 2 || total_n == 3);
...
//                     next_consonants.set(idx, consonant_survives || consonant_is_born);
//                     next_vowels.set(idx, vowel_survives || vowel_is_born);
//                 }
//             }
//         }
...
//             if !self.prev_consonants[idx] && self.consonants[idx] { self.consonant_grid[idx] = self.get_random_consonant() as u16; }
//             else if self.prev_consonants[idx] && !self.consonants[idx] { self.consonant_grid[idx] = 0; }

//             if !self.prev_vowels[idx] && self.vowels[idx] { self.vowel_grid[idx] = self.get_random_vowel() as u16; }
//             else if self.prev_vowels[idx] && !self.vowels[idx] { self.vowel_grid[idx] = 0; }
